...
this.doPasteItem = (treeItem, item, action, overwrite) => {
	var select = this.props.tree.selectedItems;
	var last_select = select[select.length - 1];
	var parent = treeItem == 'root' ? this.props.tree.data[this.props.tree.active][0] : treeItem._parent ? treeItem._parent : last_select;
	let relPath = parent.info.ext !== '' ? '' : parent.info.relativePathname;
	let path = parent.path !== '' ? parent.path.substr(1) + '/' + relPath : '/' + relPath;
	let from = this.getTreePaths(item);
	let to = parent.info.pathName + (parent.info.ext != 'php' ? '/' + parent.info.fileName : '');
	if (parent.info.ext == 'php') {
		to = to.replace(parent.info.fileName, '');
	}
	console.log(path, from, to);
	// this.pasteItem(action, path, to, overwrite, (result) => {
	// 	console.log(from, to);
	// 	console.log(result);
	// 	if (result === 'overwrite') {
	// 		this.popups.confirm.show(null, {
