...
class Redux extends Tree
{
    ...
    public function expand($path = '')
    {
        ...
        $basePath = $this->module['redux'] . $path;
        $basePath = str_replace('/', DIRECTORY_SEPARATOR, $basePath);
        $basePath = realpath($basePath);
        $rootPath = realpath($this->module['redux']);
        ...
        $finder = new Finder();
        $finder->files()->name('*Reducer.php')->in($basePath);
        $finder->depth(0);
        foreach ($finder as $f) {
            $results[] = [
                'path' => str_replace($rootPath, "", $f->getPath()),
                'label' => str_replace("Reducer.php", "", $f->getFilename()),
                'hasChild' => $f->isDir(),
                'info' => [
                    'relativePath' => $f->getRelativePath(),
                    'relativePathname' => $f->getRelativePathname(),
